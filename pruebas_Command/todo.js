/*

** Idea general **

Hacer un command center que nuclee todos los hobbys del usuario, sean estos mangas, anime, peliculas, musica, etc.
La idea sería ofrecer las siguientes funcionalidades:

*Tracking (por que capitulo vas de cada serie x ej)
*Lista de espera (cosas que todavia no se escucharon/vieron etc)
*Recomendaciones en base a autor, generos, estilos, etc
*Galeria de imagenes


**Lenguajes**

*Front-end:  Bootstrap + HTML + React + Javacript
*Back-end:  PHP + Slim


**Pasos a seguir**

1) Armar un front para la categoria anime -> Hecho
    1.a) Armar un div que tenga 4 imagenes de largo para el listado -> Hecho
    1.b) Agregar texto a cada imagen -> Hecho
    1.c) Acomodar bien el div con las imagenes -> Hecho para filas de 3

2) Armar el backend para la categoria anime
    2.a) Traer la información con la que popular el listado desde una base de datos -> Hecho en demo
    2.b) Encodear el retorno de la db en un JSON y devolover eso al AJAX -> Hecho en demo
    2.c) Renderizar el div usando la informacion que volvio de la base de datos -> Hecho en demo

3) Crear la base de datos a usar en la aplicacion -> Pendiente
    3.a) Armar tablas de prueba -> Hecho



*/