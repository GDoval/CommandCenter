...
class Pruebas
{
    ...
    public function sourceImagen() // Imagenes traidas de Internet
    {
    ...
    public function traigoImagenesDB()
    {
        ...
        $obj = array();
        $resultado = $pdo->prepare('SELECT * FROM Animes');
        $resultado->execute();
    ...
    public function guardarEnDB($nombre, $img)
    {
        $manejador = new Conexiones("127.0.0.1", "root", "ffsquall", "CommandCenter");
        try {
            $pdo = $manejador->Conexion();
        ...
        $resp = $this->guardarImagenEnDir($img);
        if ($resp == "1") {
            $path = "./img/" . $img['name'];
            try
            {
                $query = $pdo->prepare('INSERT INTO Animes (nombre, ruta) VALUES (:nombre, :ruta)');
                $query->execute(array("nombre" => $nombre, "ruta" => $path));
            } catch (PDOException $e) {
                echo $e->getMessage();
            }
        }else
        {
            echo $resp . "No entre al if de la query";
        }
    }

    private function guardarImagenEnDir($obj)
    {
        $pathOrigen = $obj['tmp_name'];

        $pathDestino = "/var/www/html/CommandCenter/img/" . $obj['name'];

        $resp = move_uploaded_file($pathOrigen, $pathDestino);
        return $resp;
    }
}
